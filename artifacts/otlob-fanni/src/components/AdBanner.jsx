...
function BannerCard({ ad, onDismiss, compact }) {
  ...
  return (
    <div className="relative rounded-[6px] overflow-hidden border border-[#FF7900]/20 bg-gradient-to-br from-[#FFF8F0] to-white shadow-sm">
      {onDismiss && (
        <button
          onClick={onDismiss}
      ...
      <a href={href} target={href.startsWith('http') ? '_blank' : '_self'} rel="noreferrer" className="block">
        {image ? (
          <div className={compact ? 'h-24' : 'h-34'}>
            <img src={image} alt={title} className="w-full h-full object-cover" />
          </div>
        ) : (
          <div className={`${compact ? 'h-24' : 'h-34'} bg-gradient-to-r from-[#071B33] to-[#1a4a7a] flex items-center justify-center px-4`}>
        ...
        {(title || desc) && image && (
          <div className="px-3.5 py-2.5 flex items-center justify-between gap-2">
            ...
            {href !== '#' && (
              <div className="flex-shrink-0 bg-[#FF7900] text-white text-xs font-bold px-3 py-1.5 rounded-[5px] flex items-center gap-1">
...
function FeaturedCard({ ad }) {
  ...
  return (
    <a
      ...
      rel="noreferrer"
      className="col-span-2 flex gap-4 items-center bg-gradient-to-r from-[#071B33] to-[#1a4a7a] rounded-[6px] p-5 shadow-md"
    >
      {image && (
        <img src={image} alt={title} className="w-20 h-20 rounded-[4px] object-cover flex-shrink-0 border-2 border-white/20" />
      )}
      ...
      <div className="flex-shrink-0 bg-[#FF7900] text-white text-xs font-bold px-4 py-2.5 rounded-[5px]">
        تفاصيل
      </div>
    </a>
  )
