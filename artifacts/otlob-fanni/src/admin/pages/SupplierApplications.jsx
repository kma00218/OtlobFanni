...
import {
  Eye, Trash2, Phone, FileText, Image, Facebook, Instagram, MapPin,
...
export default function SupplierApplications() {
  ...
  const filtered = data.filter(r => {
    ...
    const t = tab === 'all' || (tab === 'pending' && r.status === 'pending') || (tab === 'approved' && r.status === 'approved') || (tab === 'published' && r.status === 'published') || (tab === 'rejected' && r.status === 'rejected')
    return s && f && t
  })
    ...
    { key: 'requestNumber', label: 'رقم الطلب', render: (v) => <span className="text-xs font-mono text-slate-400">{v || '—'}</span> },
    {
      ...
      render: (_, row) => (
        <div className="relative">
          <button onClick={e => { e.stopPropagation(); setActionMenu(m => m === row.id ? null : row.id) }}
            className="w-7 h-7 rounded-lg bg-slate-700 hover:bg-slate-600 flex items-center justify-center text-slate-300 text-base font-bold transition-colors">⋮</button>
          {actionMenu === row.id && (
            <div className="absolute top-8 left-0 z-50 bg-slate-800 border border-slate-700 rounded-xl shadow-2xl overflow-hidden min-w-[140px]" onClick={e => e.stopPropagation()}>
              <button onClick={() => { setViewItem(row); setActionMenu(null) }} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-slate-200 hover:bg-slate-700 transition-colors">
                <Eye className="w-4 h-4" /> عرض التفاصيل
              </button>
              {row.status === 'pending' && <>
                <button onClick={() => { setStatus(row.id, 'approved'); setActionMenu(null) }} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-emerald-400 hover:bg-slate-700 transition-colors">
                  <CheckCircle className="w-4 h-4" /> قبول
                </button>
                <button onClick={() => { setRejectModal({ open: true, id: row.id, reason: '', isView: false }); setActionMenu(null) }} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-red-400 hover:bg-slate-700 transition-colors">
                  <X className="w-4 h-4" /> رفض
                </button>
              </>}
              {row.status === 'approved' && (
                <button onClick={() => { handlePublish(row.id); setActionMenu(null) }} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-orange-400 hover:bg-slate-700 transition-colors">
                  <Package className="w-4 h-4" /> نشر
                </button>
              )}
              <button onClick={() => { handleDelete(row.id); setActionMenu(null) }} className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-red-500 hover:bg-slate-700 transition-colors border-t border-slate-700">
  ...
  return (
    <div onClick={() => setActionMenu(null)}>
      ...
      {lastPublished && (
        <div className="mb-4 bg-emerald-900/40 border border-emerald-600/40 rounded-2xl px-4 py-3 flex items-center gap-3">
          <p className="flex-1 text-emerald-300 text-sm">تم نشر <strong>{lastPublished.name}</strong> — أرسل رسالة واتساب الترحيب؟</p>
          <button onClick={() => { openPublishedWhatsApp(lastPublished); setLastPublished(null) }}
            className="bg-[#25D366] text-white text-xs font-bold px-4 py-2 rounded-xl flex items-center gap-1.5">
            <WaIcon /> إرسال
          </button>
          <button onClick={() => setLastPublished(null)} className="text-slate-500 hover:text-slate-300 ml-1">
            <X className="w-4 h-4" />
          </button>
        </div>
      )}
            ...
            <div className="bg-slate-800/60 rounded-2xl p-4 space-y-3">
              ...
              {viewItem.whatsapp && viewItem.whatsapp !== viewItem.phone && (
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-slate-300 text-xs">WA: </span>
