...
import { Eye, Trash2, AlertCircle, Phone, Briefcase, Clock, FileText, Image, Lock, Facebook, Info, Shield, MessageCircle, Pencil, Save, X as XIcon } from 'lucide-react'
...
export default function TechnicianApplications() {
  ...
  const handleTechEditSave = async (e) => {
    ...
    setEditSaving(true)
    try {
      const updated = await api.admin.technicianApplications.editFields(viewItem.id, editForm)
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60" onClick={() => setActionMenu(null)}>
          <div className="bg-[#0f2236] rounded-2xl w-full max-w-xs shadow-2xl border border-slate-700 overflow-hidden" onClick={e => e.stopPropagation()}>
            <div className="px-5 pt-5 pb-3 border-b border-slate-700">
              <p className="text-white font-bold text-base text-right">{actionMenu.fullName || actionMenu.full_name || ''}</p>
            ...
            <div className="p-3 flex flex-col gap-2">
              ...
              {(actionMenu.status === 'approved' || actionMenu.status === 'rejected') && actionMenu.requestNumber && (
                <button onClick={() => { openWhatsApp(actionMenu.whatsapp || actionMenu.phone, actionMenu.fullName || actionMenu.full_name || '', actionMenu.status, actionMenu.requestNumber); setActionMenu(null) }}
          ...
          return (
            <div className="space-y-5">
              ...
              <div className="flex items-center justify-between">
                ...
                <button type="button"
                  onClick={() => editMode ? setEditMode(false) : openTechEdit()}
              ...
              {/* Edit form */}
              {editMode && (
                <form onSubmit={handleTechEditSave} className="bg-amber-50 border border-amber-200 rounded-2xl p-4 space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="text-xs text-slate-500 mb-1 block">الاسم الكامل</label>
                      <input className={inp} value={editForm.full_name} onChange={e => setEditForm(f => ({...f, full_name: e.target.value}))} />
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <select className={inp} value={editForm.city} onChange={e => setEditForm(f => ({...f, city: e.target.value}))}>
                        <option value="">— اختر</option>
                        {cities.map(c => <option key={c.id} value={c.nameAr || c.name_ar}>{c.nameAr || c.name_ar}</option>)}
                      </select>
                    </div>
                    <div>
                    ...
                    <div>
                      ...
                      <input className={inp} dir="ltr" placeholder="https://facebook.com/..." value={editForm.facebook} onChange={e => setEditForm(f => ({...f, facebook: e.target.value}))} />
                    </div>
                    <div>
                      <label className="text-xs text-slate-500 mb-1 block">إنستغرام</label>
                      <input className={inp} dir="ltr" placeholder="https://instagram.com/..." value={editForm.instagram} onChange={e => setEditForm(f => ({...f, instagram: e.target.value}))} />
                    </div>
                  </div>
                  <div>
