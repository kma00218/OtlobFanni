...
export default function Ads() {
  ...
  const [form, setForm] = useState({
    ...
    image_url: '', link_url: '', placement: 'home',
  ...
  const openAdd = () => {
    setEditItem(null)
    setForm({ title_ar: '', title_en: '', description_ar: '', description_en: '', image_url: '', link_url: '', placement: 'home', is_active: true, start_date: '', end_date: '' })
    setModalOpen(true)
  }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (v) => PLACEMENTS.find(p => p.value === v)?.label || v || '—'
    },
    {
      ...
      <FormModal open={modalOpen} onClose={() => setModalOpen(false)} title={editItem ? 'تعديل إعلان' : 'إضافة إعلان'} onSubmit={handleSubmit} loading={saving} submitLabel={editItem ? 'حفظ' : 'إضافة'} size="lg">
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div>
            ...
            <input value={form.title_ar} onChange={e => setForm(f => ({ ...f, title_ar: e.target.value }))} className="form-input" placeholder="عنوان الإعلان" />
          </div>
          <div>
            <label className="form-label">العنوان بالإنجليزي</label>
            <input value={form.title_en} onChange={e => setForm(f => ({ ...f, title_en: e.target.value }))} className="form-input" placeholder="Ad Title" dir="ltr" />
          </div>
          <div>
          ...
          <div>
            ...
            <input value={form.image_url} onChange={e => setForm(f => ({ ...f, image_url: e.target.value }))} className="form-input" placeholder="https://..." dir="ltr" />
          </div>
          <div>
            <label className="form-label">رابط الإعلان</label>
            <input value={form.link_url} onChange={e => setForm(f => ({ ...f, link_url: e.target.value }))} className="form-input" placeholder="https://..." dir="ltr" />
          </div>
          <div>
