        ...
        <div className="p-5 space-y-4">
          ...
          {image && (
            <div>
              ...
              <img src={image} alt="ad" className="w-full max-h-48 object-cover rounded-xl border border-gray-200" />
            </div>
          )}
          ...
          {link && (
            <div className="flex flex-col gap-0.5">
              ...
              <a href={link} target="_blank" rel="noreferrer" className="text-sm text-blue-600 flex items-center gap-1 hover:underline break-all">
    ...
    <div className="space-y-5" dir="rtl">
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          >
            <p className={`text-2xl font-black ${s.color === 'amber' ? 'text-amber-500' : s.color === 'green' ? 'text-green-500' : 'text-red-500'}`}>{s.count}</p>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="text-center py-12 text-gray-400">لا توجد طلبات</td></tr>
              ) : (
                filtered.map(r => {
                  ...
                  return (
                    <tr key={r.id} className="border-b border-gray-50 hover:bg-[#F7F8FA]/60 transition-colors">
                      <td className="px-4 py-3">
                        ...
                        <p className="text-xs text-gray-400">{bType}</p>
                      </td>
                      <td className="px-4 py-3 text-gray-700">{contact}</td>
                      <td className="px-4 py-3 text-gray-700 text-xs" dir="ltr">{phone}</td>
                      <td className="px-4 py-3 text-gray-600">{city}</td>
                      <td className="px-4 py-3 text-gray-600 max-w-[140px]"><span className="text-xs">{placement}</span></td>
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1.5">
                          <button onClick={() => setViewItem(r)} title="عرض التفاصيل" className="p-1.5 rounded-lg hover:bg-blue-50 text-blue-500 transition-colors"><Eye className="w-4 h-4" /></button>
                          {r.status === 'pending' && (
                            <>
                              <button onClick={() => updateStatus(r.id, 'approved')} title="قبول" className="p-1.5 rounded-lg hover:bg-green-50 text-green-500 transition-colors"><CheckCircle className="w-4 h-4" /></button>
                              <button onClick={() => updateStatus(r.id, 'rejected')} title="رفض" className="p-1.5 rounded-lg hover:bg-red-50 text-red-500 transition-colors"><XCircle className="w-4 h-4" /></button>
                            </>
                          )}
                          <button onClick={() => deleteRequest(r.id)} title="حذف" className="p-1.5 rounded-lg hover:bg-red-50 text-red-400 transition-colors"><Trash2 className="w-4 h-4" /></button>
                        </div>
                      </td>
                    </tr>
                  )
