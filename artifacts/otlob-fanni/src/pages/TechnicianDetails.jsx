    ...
    <div className="bg-[#ECEEF2] min-h-screen pt-20 pb-28" dir={ar ? 'rtl' : 'ltr'}>
      ...
      {reviewModal && (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/50 pb-0" onClick={() => { setReviewModal(false); setForm({ name: '', rating: 0, comment: '' }) }}>
          <div className="bg-white w-full max-w-lg rounded-t-3xl shadow-2xl" onClick={e => e.stopPropagation()}>
            {submitted ? (
              ...
              <div className="px-6 pt-6" style={{ paddingBottom: 'max(20px, env(safe-area-inset-bottom))' }}>
                {/* Header */}
                <div className="flex items-center justify-between mb-4">
                  <p className="font-extrabold text-[#071B33] text-lg">{ar ? 'أضف تقييمك' : 'Add Your Review'}</p>
                  <button onClick={() => { setReviewModal(false); setForm({ name: '', rating: 0, comment: '' }); setShowComment(false) }}
                ...
                {/* Name */}
                <div className="mb-3">
                  <label className="block text-xs font-bold text-gray-600 mb-1.5">{ar ? 'اسمك *' : 'Your name *'}</label>
                  <input
                    dir={ar ? 'rtl' : 'ltr'}
                    type="text"
                ...
                {!showComment ? (
                  <button
                    ...
                    className="text-xs font-bold text-[#FF7900] mb-4 flex items-center gap-1 active:opacity-70">
                    <span>+</span>
                    <span>{ar ? 'أضف تعليقاً (اختياري)' : 'Add a comment (optional)'}</span>
                  </button>
                ) : (
                  <div className="mb-4">
                    <label className="block text-xs font-bold text-gray-600 mb-1.5">{ar ? 'تعليقك' : 'Comment'}</label>
                    <textarea
                      dir={ar ? 'rtl' : 'ltr'}
                      rows={3}
